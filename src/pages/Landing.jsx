...
export default function Landing({ data }) {
    ...
    const line1 = zh.slice(0, 6);
    const line2 = zh.slice(6, 10);
    const en = (data.name.en).replace("(simple version)", "");

    return (
        <div>
            <section className={`w-full h-135 bg-center bg-cover bg-no-repeat ${homeBannerBg} max-[700px]:h-70`}>
            </section>
            ...
            <section className="w-full max-w-370 mx-auto py-24 px-5 bg-white-900">
                <div className="flex flex-col gap-18.75 max-[600px]:gap-10">
                    ...
                    <div className=" flex justify-end text-left gap-7.5 max-[870px]:flex-col">
                        <p
                            className="max-w-112.5 text-left text-black-700 ">
                            {data.description}
                        </p>
